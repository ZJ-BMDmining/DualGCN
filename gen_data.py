...
def get_parser(parser=None):
    ...
    parser.add_argument('-q','--quantile',type=float,default='0.6')
    return parser
...
#     graph_df = pd.read_csv(net_file,header=None,index_col=None , delimiter='\t')
    graph_df = pd.read_csv(net_file,header=0,index_col=None,delimiter="\t")
    graph_df.columns = ['node1', 'node2', 'score']
    graph_df = graph_df.loc[graph_df.score.ge(graph_df.score.quantile(0.6)).values,['node1','node2']] # quantile 0.6,设置一个阈值，将score大于0.6的边保留，变为1，其余的直接删除
